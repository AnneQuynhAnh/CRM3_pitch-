document.addEventListener("DOMContentLoaded", function () {
  ...
  function populateEditPaymentSection(order) {
    ...
    <div id="vatInvoiceSection" style="margin-top: 10px;">
      <p>
        ...
        <input type="text" id="vatNumber" value="${
          order.VAT_number || ""
        }" placeholder="Nhập mã đơn VAT" style="color: black;" />
      </p>
    </div>
    ...
    <div id="cashDetails" class="${
      ...
      <p>Thời gian: <input type="date" id="timeReceiveCash" value="${
        order.payment_timing_2
          ? new Date(order.payment_timing_2).toISOString().split("T")[0]
          : ""
      }" min="${today}" /></p>
      <p>Tiền đặt cọc: <input type="number" id="depositedAmountCash" value="${
        order.deposited_2 || 0
      }" /></p>
    </div>
    ...
    <div id="transactionDetails" class="${
      ...
      <p>Thời gian: <input type="date" id="timeTransaction" value="${
        order.payment_timing_2
          ? new Date(order.payment_timing_2).toISOString().split("T")[0]
          : ""
      }" min="${today}" /></p>
      <p>Tiền đặt cọc: <input type="number" id="depositedAmountTransaction" value="${
        order.deposited_2 || 0
      }" /></p>
      <p>Ngân hàng:</p>
    ...
    <button id="editUpdateButton" style="background-color: #ff4a4a; color: white; border: none; padding: 10px 20px; cursor: pointer; font-size: 16px; display: block; margin: 20px auto;">Cập nhật Thanh Toán</button>
  `;
  ...
  function handleEditPaymentUpdate(order) {
    ...
    // Retrieve note details and VAT number
    ...
    const paymentData = {
      ...
      VAT_number: vatNumber, // Send VAT Number to the VAT_number column
    };
  ...
  function showDeliveryDetailsPopup(order) {
    ...
    modalContent.innerHTML = `
      ...
      <div id="deliveryDetails">
        <p>Địa chỉ <input type="text" id="deliveryAddress" /></p>
        <p>Phí giao hàng <input type="number" id="deliveryFee" /></p>
        <p>Note: <input type="text" id="deliveryNote" /></p>
      </div>
      <button id="submitDeliveryDetails" style="background-color: #4CAF50; color: white; border: none; padding: 10px 20px; cursor: pointer; font-size: 16px; display: block; margin: 20px auto;">Xác nhận</button>
    `;
